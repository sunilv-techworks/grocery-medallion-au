...
- **Orchestration platform:** `ws-grocery-orchestration-dev` holds the control plane — `pl_grocery_medallion`, a metadata-driven pipeline (SeedMetadata → BronzeDims → SilverDims → GoldDims), plus a `config.table_metadata` table that drives the runner notebooks and a shared `runs.pipeline_runs` table that each layer logs start/success/failure to. The generic Bronze/Silver/Gold runner notebooks themselves live in `ws-grocery-engineering-dev`, invoked cross-workspace by the pipeline.
